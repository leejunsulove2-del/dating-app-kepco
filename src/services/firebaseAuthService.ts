 ...
 */
export function validateEmailDomain(email: string): { isValid: boolean; domain: string; companyName?: string; error?: string } {
  ...
  const matched = DEFAULT_ALLOWED_DOMAINS.find((d) => d.domain.toLowerCase() === domain);

  if (!matched) {
    return {
      isValid: false,
      domain,
      error: `@${domain} 은(는) 등록 가능한 기관 도메인이 아닙니다. 공무원(@korea.kr) 또는 한국전력공사(@kepco.co.kr) 등 허용된 11개 공공기관 이메일만 가입 가능합니다.`,
    };
  }
...
export async function registerWithEmailVerification(
  ...
  if (auth) {
    try {
      // 1) 유저 계정 생성
      const userCredential = await createUserWithEmailAndPassword(auth, cleanEmail, passwordPlain);
      ...
      return {
        ...
        message: `${cleanEmail} 계정으로 Firebase 인증 링크 메일이 발송되었습니다. 메일함에서 링크를 클릭하여 인증을 완료해주세요.`,
      };
    } catch (err: unknown) {
      const fbError = err as { code?: string; message?: string };
      if (fbError.code === 'auth/email-already-in-use') {
        throw new Error('이미 가입된 이메일 주소입니다. 로그인 탭에서 로그인을 진행해주세요.');
  ...
  const simulatedLink = `${window.location.origin}/?verify_token=${simulatedToken}&email=${encodeURIComponent(cleanEmail)}`;
