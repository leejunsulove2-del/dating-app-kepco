...
// 💡 파이어베이스에 직접 아이템 재화를 실시간 동기화하기 위해 모듈을 추가합니다.
...
export class ItemService {
 ...
 /**
  * 💡 로컬 스토리지에 인벤토리를 저장하는 것과 동시에 파이어베이스 클라우드로 
  * 즉각 덮어씌워 주는 동기화 핵심 보안 로직을 완전 개방 형태로 강제 매핑합니다.
  */
 ...
 // GitHub Pages 환경에서도 정상 작동하도록 Firestore 다이렉트 동기화 세팅
 if (firestoreDb) {
   setDoc(doc(firestoreDb, 'user_inventories', userId), {
     ...inventory,
     updatedAt: serverTimestamp()
   }, { merge: true }).catch((err) => console.error('클라우드 DB 인벤토리 동기화 실패:', err));
 }
 ...
 const actionText = type === 'up' ? '올렸습니다 (+1) 🌲' : '내렸습니다 (-1) ❄';
 return {
 success: true,
 ...
 const message = res.action === 'attached'
 ...
 /**
  * 💡 7. 관리자 전송 및 이벤트 직접 지급 도우미 (Admin & Event Direct Grant Helpers)
  * 수량 가산 후 상단에서 커스텀 설계한 파이어베이스 다이렉트 saveInventory를 경유시켜 
  * 관리자 포털에서 아이템 지급 즉시 회원의 모바일 클라우드 DB에도 실시간 반영을 완수합니다.
  */
